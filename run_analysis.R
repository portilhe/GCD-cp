...
#Merge train and test data, in this order, and get rid of the separate variables
X = rbind(X_train,X_test)
Y = rbind(Y_train,Y_test)
subject = rbind(subject_train,subject_test)
rm(X_test, X_train, Y_test, Y_train, subject_test, subject_train)

#Rename the columns in the data X.
features = gsub("\\(\\)","",features[,2])
colnames(X) = features

#Keep only the columns corresponding to mean and standard deviation values
...
#Bind the subject and activities vectors to the data set and rename this two
...
#Sort the tidy data set by Subject and Activity
require(plyr)
